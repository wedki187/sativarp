window.APP = {
  ...
  methods: {
    ON_OPEN() {
      this.showInput = true;
      this.showWindow = true;
      post('http://chat/statuschatu', JSON.stringify({status: "on"}));
      if (this.showWindowTimer) {
        clearTimeout(this.showWindowTimer);
      }
    ...
    keyDown(e) {
      if (e.which === 38 || e.which === 40) {
        ...
        $('.chat-input > textarea').emojiPicker('toggle');
      }
    },
    ...
    hideInput(canceled = false) {
      ...
      post('http://chat/statuschatu', JSON.stringify({status: "off"}));	  
      this.message = '';
